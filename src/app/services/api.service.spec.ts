...
describe('ApiService', () => {
  ...
  it('should make a GET request to fetch user data', () => {
    service.getUser('Anamika024').subscribe((response) => {
      expect(response.body).toEqual(mockUser);
    });

    const req = httpMock.expectOne('https://api.github.com/users/Anamika024');
    expect(req.request.method).toBe('GET');
    req.flush(mockUser);
  });
  
  it('should make a GET request to fetch user repositories', () => {
    service.getRepos('Anamika024', 1, 10).subscribe((response) => {
      expect(response.body).toEqual(mockRepoData);
    });

    const req = httpMock.expectOne(
      'https://api.github.com/users/Anamika024/repos?sort=updated&page=1&per_page=10'
    );
    expect(req.request.method).toBe('GET');
